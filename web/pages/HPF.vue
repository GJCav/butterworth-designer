  ...
  <v-container>
    ...
    <v-row class="mt-2 align-baseline" dense>
      <v-col cols="6">
        <v-row class="align-baseline">
          <v-col cols="3" style="text-align: right;">
            <span class="text-h6">\(G_\infty = \)</span>
          </v-col>
          <v-col cols="9">
            <v-text-field v-model="G_0_i" label="高频增益" outlined dense></v-text-field>
          </v-col>
        </v-row>
      </v-col>
      <v-col cols="6">
    ...
    <v-container class="pl-4">
          ...
          <v-row class="mx-0">
            <v-col class="justify-center">
              <div style="width: 500px; height: 500px;">
                <HPFMagRes :order="N" :cutoff="w_c" :dc-gain="G_0_val"></HPFMagRes>
              </div>
            </v-col>
            <v-col class="justify-center">
              <div style="width: 500px; height: 500px;">
      ...
      <!-- 电路结构、参数 -->
      <v-row ref="doc_params" class="align-center mt-2">
      ...
      <v-expand-transition>
        <v-container v-show="expand_params" fluid class="mt-2">
          ...
          <v-row style="width: 80%; margin: 0 auto 0 auto"><v-img src="/hpf.jpg" contain /></v-row>
          <v-row class="my-0">
  ...
  methods: {
    solve_filter() {
      ...
      if (this.enable_resp_chart) {
        ...
        const H = BTW.hpf_transfer_func(data)
      ...
      const params = BTW.calc_filter_param(
        data, 
        [
          { C1: 1e-14, R3: 10000 },
          { C1: 1e-14, R3: 10000 },
      ...
      // convert to HPF params
      for (let i = 0; i < params.length; i++) {
        const lpf = params[i]
        const hpf = { ...params[i] }
        if (lpf.order === 1) {
          hpf.C1 = 1 / lpf.R1
          hpf.R1 = 1 / lpf.C1 / this.w_c
        } else {
          hpf.C1 = 1 / lpf.R1
          hpf.C2 = 1 / lpf.R2
          hpf.R1 = 1 / lpf.C1 / this.w_c
          hpf.R2 = 1 / lpf.C2 / this.w_c
        }
        params[i] = hpf
      }
